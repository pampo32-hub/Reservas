...
class App {
      ...
      <div class="bg-white rounded-3xl border ${isBlocked ? 'border-rose-300 ring-2 ring-rose-500/20 shadow-md bg-rose-50/10' : (isUnlimited ? 'border-purple-300 ring-2 ring-purple-500/10 shadow-md' : isPro ? 'border-amber-300 shadow-sm' : 'border-slate-200 shadow-xs')} overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col group hover:-translate-y-1 relative">
        ...
        <div class="p-5 sm:p-6 flex-1 flex flex-col justify-between">
          <div>
              ...
              ${biz.services && biz.services.length > 0 
                ? biz.services.slice(0, 2).map(s => `
                  <div class="flex justify-between items-center text-xs py-0.5">
                    ...
                    <span class="font-bold text-blue-600 shrink-0">${this.formatColones(s.price || 0)}</span>
                  </div>
                `).join('')
            ...
            <!-- Schedule & Count -->
            <div class="mt-3 flex items-center justify-between text-xs text-slate-600 pt-3 border-t border-slate-100">
              <span class="flex items-center gap-1.5">
                <i class="far fa-clock text-blue-500"></i>
                ${biz.schedule ? `${this.formatTime12h(biz.schedule.openTime)} - ${this.formatTime12h(biz.schedule.closeTime)}` : 'Horario flexible'}
              </span>
        ...
        <div class="cal-desktop-view hidden md:block border rounded-3xl overflow-hidden ${theme.desktopContainer} shadow-2xs transition-colors">
          ...
          <div class="grid grid-cols-7 divide-x divide-y ${theme.desktopGridDivide}">
            ${cells.map(cell => {
              ...
              let cellBgClass = '';
              if (cell.isToday) {
                cellBgClass = theme.desktopCellToday;
  ...
  renderTeamTabContent(currentBiz) {
    ...
    const isLimitReached = isPro && staffList.length >= 5;
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            <button 
              ...
              class="px-4 py-2.5 ${isLimitReached ? 'bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-black' : 'bg-blue-600 hover:bg-blue-700 text-white font-bold'} rounded-xl text-xs flex items-center gap-2 transition-all shadow-md shadow-blue-500/20 cursor-pointer active:scale-95"
            >
              <i class="fas ${isLimitReached ? 'fa-crown text-amber-900' : 'fa-user-plus'}"></i>
              <span>${isLimitReached ? 'Límite 5/5 (Agregar más)' : 'Agregar Especialista'}</span>
            </button>
          </div>
        </div>
  ...
  renderStaffModal(business, staffMember = null) {
    ...
    const staffList = storage.getBusinessStaffSync(business?.id) || [];

    if (!isEdit && (plan === 'free' || plan === 'basic')) {
      this.renderStaffLimitModal(business, 'basic');
      return;
    }

    if (!isEdit && plan === 'pro' && staffList.length >= 5) {
      this.renderStaffLimitModal(business, 'pro');
      return;
    }
    ...
    document.getElementById('staff-form')?.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (err.message && (err.message.includes('límite') || err.message.includes('5 especialistas') || err.message.includes('requiresUpgrade') || err.message.includes('Ilimitado') || err.message.includes('Profesional') || err.message.includes('múltiples especialistas'))) {
          modalContainer.innerHTML = '';
          this.renderStaffLimitModal(business, business.plan || 'pro');
        } else {
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              ${slots.map(slot => {
                ...
                if (slot.status === 'break') {
                  return `
                    <div class="p-3.5 rounded-2xl border border-slate-200 bg-slate-100 text
  ...
  // --- MODAL DE LÍMITE DE ESPECIALISTAS (PLAN PRO: MÁXIMO 5 / PLAN BÁSICO: 1) ---
  renderStaffLimitModal(currentBiz, limitType = 'pro') {
    const modalContainer = document.getElementById('modal-container');
    if (!modalContainer) return;

    const isBasicOrFree = limitType === 'basic' || currentBiz?.plan === 'free' || currentBiz?.plan === 'basic';
    const currentCount = storage.getBusinessStaffSync(currentBiz?.id)?.length || (isBasicOrFree ? 1 : 5);
      ...
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop animate-fade-in">
        <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-200 p-6 sm:p-7 text-center">
          ...
          <h3 class="text-xl font-black text-slate-900">
            ${isBasicOrFree ? '¿Deseas agregar especialistas a tu equipo?' : '¿Deseas agregar más de 5 especialistas?'}
          </h3>
          <p class="text-xs text-slate-600 mt-2 leading-relaxed">
            ${isBasicOrFree 
    ...
    document.getElementById('close-staff-limit-btn')?.addEventListener('click', () => {
      modalContainer.innerHTML = '';
    });

    document.getElementById('upgrade-from-staff-limit-btn')?.addEventListener('click', () => {
      modalContainer.innerHTML = '';
      this.renderPlansModal({ businessId: currentBiz?.id, currentPlanId: isBasicOrFree ? 'basic' : 'pro' });
    });
  }
